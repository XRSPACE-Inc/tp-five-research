...
namespace
{
    ...
    __global__ void rasterizer_forward_cuda_kernel(
        const float* face_verts,
        const int num_faces,
        float blur_radius,
        int N,
        ...
        float* bary)
    {
        ...
        for (int i = tid; i < N * H * W; i += num_threads)
        {
            ...
            const int face_start_idx = n * num_faces;
            const int face_stop_idx = face_start_idx + num_faces;
    ...
    __global__ void rasterizer_backward_cuda_kernel(
        const float* face_verts, // (N, F, 3, 3)
        ...
        float* grad_face_verts) // (N, F, 3, 3)
    {
        ...
        for (int t_i = tid; t_i < N * H * W; t_i += num_threads)
        {
            ...
            for (int k = 0; k < K; k++)
            {
                ...
                const int i = n * H * W * K + yi * H * K + xi * K + k; // pixel index + face index
...
{
    if (face_verts.ndimension() != 4 || face_verts.size(2) != 3 ||
        face_verts.size(3) != 3) {
        AT_ERROR("face_verts must have dimensions (batch_size, num_faces, 3, 3)");
    }
    ...
    const int num_faces = face_verts.size(1);
    const int N = batch_size; // batch size.
    ...
    at::Tensor face_idxs = torch::full({N, H, W, K}, -1, long_opts);
    at::Tensor zbuf = torch::full({N, H, W, K}, -1, float_opts);
    at::Tensor pix_dists = torch::full({N, H, W, K}, -1, float_opts);
    at::Tensor bary = torch::full({N, H, W, K, 3}, -1, float_opts);

    const size_t blocks = 1024;
    const size_t threads = 64;

    rasterizer_forward_cuda_kernel<<<blocks, threads>>>(
...
at::Tensor rasterizer_backward_cuda(
    const at::Tensor& face_verts, // (N, F, 3, 3)
    ...
    const at::Tensor& grad_dists) // (N, H, W, K))
{
    ...
    at::Tensor grad_face_verts = torch::zeros({N, F, 3, 3}, face_verts.options());
    const size_t blocks = 1024;
    const size_t threads = 64;

    rasterizer_backward_cuda_kernel<<<blocks, threads>>>(
        face_verts.contiguous().data<float>(),
