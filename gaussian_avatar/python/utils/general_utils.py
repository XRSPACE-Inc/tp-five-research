...
def PILtoTensor(pil_image):
    ...
    resized_image = torch.from_numpy(im) / 255.0
    if len(resized_image.shape) == 3:
        return resized_image.permute(2, 0, 1)
    else:
        return resized_image.unsqueeze(dim=-1).permute(2, 0, 1)
...
def antialias_warp_affine(image: np.ndarray, mat: np.ndarray, target_size: tuple[float, float], **kwargs) -> np.ndarray:
    assert image.dtype in [np.uint8, np.float32, np.float64]
    orig_shape = image.shape
    tW, tH = (int(target_size[0]), int(target_size[1]))
    image = cv2.warpAffine(image, mat[:2], (tW, tH), **kwargs).reshape(tH, tW, *orig_shape[2:])
    assert len(image.shape) == len(orig_shape)
    return image


def PILtoTensorCropped(pil_image, crop_data, target_size):
    # pil_image = pil_image.transpose(Image.FLIP_TOP_BOTTOM)
    im = np.array(pil_image)
    if im.dtype == bool:
        im = im.astype(np.uint8) * 255
    im = antialias_warp_affine(im, crop_data[:2, :], target_size)
    resized_image = torch.from_numpy(im) / 255.0
    if len(resized_image.shape) == 3:
        return resized_image.permute(2, 0, 1)
    else:
