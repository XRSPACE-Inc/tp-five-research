...
class Deform_Model(nn.Module):
    def __init__(self, device, bs_size: int=120, use_same_shape: bool=False, lite_model: bool=True):
        super().__init__()
        self.device = device
        ...
        self.init_networks(bs_size, lite_model)

    def init_networks(self, linear_size=120, lite_model=False, hidden_layers=6):
        # full mica
        self.deformNet = MLP(
            input_dim=self.pts_embedder.dim_embeded+linear_size,
            output_dim=10,
    ...
    def example_init(self, codedict):
        ...
        # self.default_shape = face_vertices_shape.detach().clone()
        ...
        uv_vertices_shape_embeded = self.pts_embedder(uv_vertices_shape)
        ...
        self.uv_vertices_shape_embeded = uv_vertices_shape_embeded[:, self.uv_head_idx, :]
    ...
    def decode(self, codedict):
        ...
        batch_size = shape_code.shape[0]
        if 'bs_param' in codedict:
            condition = codedict['bs_param']
        else:
        ...
        # torch.exp(x) ~= torch.pow(2, x * 1.44269504)

        # rot_delta_0 = deforms[..., 3:7]
        # rot_delta_r = torch.pow(2, rot_delta_0[..., 0:1] * 1.44269504)
        ...
        if self.use_same_shape and self.default_shape is not None:
            ...
            uv_vertices = uv_vertices[:, self.uv_head_idx, :]
    ...
    def __init__(self, input_dim, condition_dim, output_dim1, output_dim2, hidden_dim=256, hidden_layers=8):
        ...
        self.fcs1 = nn.ModuleList(
            [nn.Linear(input_dim, hidden_dim)] + [
                nn.Linear(hidden_dim, hidden_dim) if i != (mid_layers-1) else nn.Linear(hidden_dim, hidden_dim+output_dim1) for i in range(mid_layers)
            ]
        )
