...
from scene import GaussianModel, Scene_landmark
from src.landmark_model import SphereModel as Deform_Model
...
def setup_scene(args, lpt, opt, train_type):
    # deform model
    DeformModel = Deform_Model(args.device, 0, use_same_shape=True, lite_model=False).to(args.device)
...
def main():
    ...
    DeformModel, scene, gaussians, _, train_dir, model_dir = setup_scene(
        args,
        lpt,
        opt,
    ...
    visualize_iteraions = 5 if args.debug else 500
    print('======== start')
    for iteration in range(first_iter, total_iteraions):
        ...
        codedict['B'] = viewpoint_cam.bs_param.shape[0]
