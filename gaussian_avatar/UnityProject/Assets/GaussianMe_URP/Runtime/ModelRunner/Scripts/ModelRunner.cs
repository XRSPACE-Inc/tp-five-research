...
namespace GaussianMe
{
    public class ModelRunner : MonoBehaviour
    {
        ...
        IEnumerator Start()
        {
            ...
            while (!ortRunner.IsReady || !setupDone)
            {
                yield return null;
            }
        ...
        public void UpdateBlendshapeInput(float[] values)
        {
            foreach (var (index, arrIndex) in indexMap)
            {
                blendshapes[arrIndex] = values[index];
            }
            if (ict)
            {
